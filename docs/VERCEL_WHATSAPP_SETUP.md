...
Aucune migration SQL. Après mise à jour du code local (`git pull --ff-only`), relancer `npm run marketing:sync` pour remplacer atomiquement la copie livrée par la copie tous statuts. Le garde-fou d’une heure entre imports reste actif. Le plafond devient 250 000 lignes, avec une pause de 20 ms par bloc de 2 000 ; le budget global de 60 secondes, les délais de requête et la connexion source unique restent en place. En cas de dépassement, la copie précédente est conservée. Une validation locale ne garantit pas le temps du premier import tous statuts sur l’ERP réel.
